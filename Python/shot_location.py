...
df = df.loc[df['Player Name'] == "LeBron James"]
df.head()

# Définissez le style et les codes de couleur
sns.set_style("white")
sns.set_color_codes()
...
plt.scatter(df["X Location"], df["Y Location"])
...
# Tracé des données
plt.scatter(data["X Location"], data["Y Location"])
draw_court(ax)

# Ajustement des limites du graphique pour ne montrer que la moitié du terrain
plt.xlim(-250, 250)
plt.ylim(422.5, -47.5)

# Masquage des étiquettes des axes
ax.set_xticks([])
ax.set_yticks([])
